...
function Dashboard({ session }: { session: Session }) {
  ...
  async function load() {
    ...
    const firstError = t.error || l.error || c.error
    setLoadError(firstError?.message || '')
    setTx((t.data || []) as Tx[]); setLoans((l.data || []) as Loan[]); setCards((c.data || []) as Card[])
  }
  useEffect(() => { void load() }, [])
  ...
  async function addTransaction(event: FormEvent) {
    ...
    const { error } = await supabase.from('transactions').insert({ id: newTransactionId(), user_id: session.user.id, kind, amount: Number(amount), category, note, date: transactionDate })
    if (error) return alert(error.message)
    setAmount(''); setNote(''); setTransactionOpen(false); await load()
  }
  ...
  {transactionOpen && <div className="modal-backdrop" onMouseDown={() => setTransactionOpen(false)}><form className="modal creation-modal" onSubmit={addTransaction} onMouseDown={e => e.stopPropagation()}><header><h2>Add transaction</h2><button type="button" className="plain-icon" onClick={() => setTransactionOpen(false)}><X /></button></header><div className="segmented"><button type="button" className={kind === 'expense' ? 'active' : ''} onClick={() => { setKind('expense'); setCategory('Food') }}>Expense</button><button type="button" className={kind === 'income' ? 'active' : ''} onClick={() => { setKind('income'); setCategory('Salary') }}>Income</button></div><label>Amount<input type="number" min="0.01" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} placeholder="₱ 0.00" required /></label><label>Category<select value={category} onChange={e => setCategory(e.target.value)}>{(kind === 'income' ? ['Salary', 'Freelance', 'Business', 'Other'] : ['Food', 'Bills Payment', 'Transport', 'Shopping', 'Health', 'Other']).map(item => <option key={item}>{item}</option>)}</select></label><label>Description<input value={note} onChange={e => setNote(e.target.value)} placeholder="What was this for?" /></label><label>Transaction date<input type="date" value={transactionDate} onChange={e => setTransactionDate(e.target.value)} required /></label><button className="workspace-primary">Save {kind}</button></form></div>}
